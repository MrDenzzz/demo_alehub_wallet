<template>
    <div class="activityList">
        <div v-for="(activity, activityIndex) in activities"
             :key="activityIndex" class="activityListBlock">

            <panel-heading :title="showDate(activity.timestamp)"
                           v-if="checkShow(activityIndex) && isShowDate"/>

            <div class="activity-list">
                <div class="status sent"
                     v-if="typeTransaction(activity.walletAddress)">
                ...
                <div class="details"
                     :class="{ 'sent': typeTransaction(activity.walletAddress),
                               'received': !typeTransaction(activity.walletAddress) }">
                    <div class="info">
                ...
                <div class="result-day-col">
                    <div class="result-day-line">
                        <span class="result-day-span">
                            <span class="count">
                                <formatting-price :value="resultReceived(activity.timestamp)"/>
                            </span>
                            ALE
                        </span>
                    </div>
                    ...
                    <div class="result-day-line">
                        <span class="result-day-span sent">
                            <span class="count">
                                 <formatting-price :value="resultSent(activity.timestamp)"/>
                            </span>
                            ALE
                        </span>
                    </div>
                    ...
                    <div class="result-day-line">
                        <span class="result-day-span">
                            <span class="count">
                                <formatting-price :value="resultTotal(activity.timestamp)"/>
                            </span>
                            ALE
                        </span>
                    </div>
        ...
        methods: {
            ...
            checkShow: function (index) {
                ...
                if (index === this.activities.length)
                    return !(Moment(this.activities[index].timestamp).format('YYYY/MM/DD') === Moment(
                        this.activities[index + 1].timestamp).format('YYYY/MM/DD'));
                else
                    return !(Moment(this.activities[index].timestamp).format('YYYY/MM/DD') === Moment(
                        this.activities[index - 1].timestamp).format('YYYY/MM/DD'));
            },
            ...
            typeTransaction: function (address) {
                 return address === this.currentWallet.address;
            }
        },
        created() {
        },
